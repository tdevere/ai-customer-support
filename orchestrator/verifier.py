...
class VerifierAgent:
    ...
    def verify(
        ...
        tools_text = self._format_tools(tool_results) if tool_results else "No tools used"
...
Verify this response and provide your assessment."""
    ...
    def _format_sources(self, sources: List[Dict[str, Any]]) -> str:
        ...
        for i, source in enumerate(sources, 1):
            formatted.append(f"[{i}] {source.get('title', 'Untitled')}: {source.get('content', '')[:200]}...")
        return "\n".join(formatted)
    ...
    def _format_tools(self, tool_results: List[Dict[str, Any]]) -> str:
        ...
        for tool in tool_results:
            formatted.append(f"- {tool.get('tool', 'Unknown')}: {tool.get('result', 'N/A')}")
        return "\n".join(formatted)
    
    def _parse_verification(self, verification_text: str, agent_confidence: float) -> Dict[str, Any]:
        ...
        result["should_escalate"] = (
            result["final_confidence"] < settings.confidence_threshold or
            result["grounded"] == "no" or
            len(result["concerns"]) > 2
        )
