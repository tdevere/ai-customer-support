...
@tool
async def create_refund(order_id: str, amount: float, reason: str = "customer_request") -> Dict[str, Any]:
    """
    ...
    url = f"{settings.shopify_shop_url}/admin/api/2024-01/orders/{order_id}/refunds.json"
    headers = {
        "X-Shopify-Access-Token": settings.shopify_api_key,
        "Content-Type": "application/json"
    }
...
async def check_return_eligibility(order_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        eligible = (
            ...
            fulfillment_status in ["fulfilled", "partial"]
        )
        ...
        return {
            ...
            "reason": "Within 30-day return window" if eligible else "Outside return window or not fulfilled"
        }
