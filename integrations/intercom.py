...
async def intercom_webhook(request: Request):
    ...
    signature = request.headers.get("X-Hub-Signature-256") or request.headers.get("X-Intercom-Signature")
    ...
    if topic in ["conversation.user.replied", "conversation.user.created"]:
        ...
        if result.get("status") == "success" and result.get("confidence", 0) >= settings.confidence_threshold:
